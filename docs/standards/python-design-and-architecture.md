...
These standards apply to Python code in `kaos-ml-core`.

`kaos-ml-core` publishes the `kaos_ml_core` import package. This package publishes CLI entry point(s): `kaos-ml`, `kaos-ml-serve`.

## Package Shape

- Keep the import package name aligned with the distribution name:
  `kaos-ml-core` publishes import package `kaos_ml_core`.
- Declare the public API in `kaos_ml_core.__all__` when the package has
...
- Names exported from `kaos_ml_core.__all__`.
...
- Put optional integrations in named extras. Public extras currently declared: `llm`, `mcp`, `transformers`.
